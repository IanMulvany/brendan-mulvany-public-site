...
document.addEventListener('DOMContentLoaded', () => {
    initializeAuth();
    setupEventListeners();
    loadImages({ reset: true }).catch(err => console.error('Initial load failed:', err));
});
...
async function loadImages({ reset = false } = {}) {
    ...
    const staticData = window.__STATIC_DATA__;
    if (staticData && reset && Array.isArray(staticData) && staticData.length > 0) {
        // Use static data - no API call needed
        container.innerHTML = '';
        renderGalleryPage(staticData, false);
        allImageIds = staticData.map(img => img.image_id);
        hasMore = true; // Assume more for now, or check against total count if available
        currentPage = 1; // Next page would be page 1
        updateLoadMoreState();
        return;
    }
...
function renderGalleryPage(images, append = false) {
    ...
    if (!Array.isArray(images)) {
        console.error('renderGalleryPage expected array, got:', typeof images);
        return;
    }
